...
ethyl_data <- gas_vocs %>% filter(type == "Ethylbenzene") %>% 
...
voc_predictors <- merge(ethyl_data,
                          predictors, by = "ID")

voc_predictors[is.na(voc_predictors)] <- 0

# Ethylbenzene predictors -----

ethyl_predictors <- voc_predictors %>% select("ID", "X_km", "Y_km", 
                                                     "Dec 2005","Apr 2006", 
                                                     "Aug 2006",
                                                     "Av_NOx_500m",
                                                     "Av_NOx_1000m",
...
ethyl_predictors[,c("X_km", "Y_km")] <- scale(ethyl_predictors[,c("X_km", "Y_km")])
dist_matrix <- dist(ethyl_data[c("X_km", "Y_km")]) %>% as.matrix()

campaigns_indx <- which(colnames(ethyl_predictors) %in% c("ID","Dec 2005", "Apr 2006", "Aug 2006"))
ethyl_predictors[,-campaigns_indx] <- scale(ethyl_predictors[,-campaigns_indx])

# Prepare data for models

Nsites <- nrow(ethyl_predictors)

ones <- rep(1,Nsites) %>% as.data.frame()
D1 <- c(0,1,0)
D2 <- c(0,0,1)

covs <- data.frame(ones, ethyl_predictors[,-c(1,4:6)])
...
mcmc.out <- runMCMC(CmodelMCMC, niter = 20000,
                    nburnin = 1000, thin = 15, nchains = 2,
...
spatialCodeinits <-list( "beta" = rnorm(ncol(covs)), "phi" = rexp(3,0.3), 
                         "tau2" = rinvgamma(1,2,1),
                         "sigma2" = rinvgamma(3,2,1), 
                         "psi2" = rinvgamma(1,2,1))


model <- nimbleModel(lod_allcampaigns3, 
...
plot(mcmc.out4$samples$chain1[ , 'sigma2[1]'], type = 'l', xlab = 'iteration', 
     ylab = expression(sigma[1]^2), bty="n")
lines(mcmc.out4$samples$chain2[ , 'sigma2[1]'], col = "red")

plot(mcmc.out4$samples$chain1[ , 'sigma2[2]'], type = 'l', xlab = 'iteration', 
     ylab = expression(sigma[2]^2), bty="n")
lines(mcmc.out4$samples$chain2[ , 'sigma2[2]'], col = "red")

plot(mcmc.out4$samples$chain1[ , 'sigma2[3]'], type = 'l', xlab = 'iteration', 
     ylab = expression(sigma[3]^2), bty="n")
lines(mcmc.out4$samples$chain2[ , 'sigma2[3]'], col = "red")
...
plot(mcmc.out4$samples$chain1[ , 'phi[1]'], type = 'l', xlab = 'iteration', 
     ylab = expression(phi[1]), bty="n")
lines(mcmc.out4$samples$chain2[ , 'phi[1]'], col = "red")

plot(mcmc.out4$samples$chain1[ , 'phi[2]'], type = 'l', xlab = 'iteration', 
     ylab = expression(phi[2]), bty="n")
lines(mcmc.out4$samples$chain2[ , 'phi[2]'], col = "red")

plot(mcmc.out4$samples$chain1[ , 'phi[3]'], type = 'l', xlab = 'iteration', 
     ylab = expression(phi[3]), bty="n")
lines(mcmc.out4$samples$chain2[ , 'phi[3]'], col = "red")

# Parameter summary -----

varnames <- c("tau2","phi[1]", "phi[2]","phi[3]","phi[4]",
              "sigma2[1]", "sigma2[2]", "sigma2[3]", "sigma2[4]")

summary_coef <- mcmc.out4$summary$all.chains[varnames, ] %>% as.data.frame()


summary_coef$variable <- as.factor(rownames(summary_coef))


xtable(summary_coef[,c("Mean", "95%CI_low", "95%CI_upp")]) 

# Coefficient Estimates beta
coeff_estimates <- mcmc.out4$summary$all.chains[grepl("beta" , 
...
mcmc.out4 <- readRDS(file = "ethyl_lur_M4.rds")
...
plot_dec <- ggplot(data = fitted_real_dec) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        axis.line.y.right = NULL) + xlim(c(0,3.2)) + ylim(c(0,3.2)) + 
...
plot_april <- ggplot(data = fitted_real_apr) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.border = element_blank(),
        panel.background = element_blank(),
        axis.line.y.right = NULL)  + xlim(c(0,2.5)) + ylim(c(0,2.5)) + 
  geom_abline(slope=1, intercept = 0) + ylab("fitted") + xlab("observed")+ 
  ggtitle("April 2006")
...
plot_august <- ggplot(data = fitted_real_aug) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.border = element_blank(),
        panel.background = element_blank(),
        axis.line.y.right = NULL) + xlim(c(-1,3.5)) + ylim(c(-1,3.5)) + 
  geom_abline(slope=1, intercept = 0) + ylab("fitted") + xlab("observed")+ 
  ggtitle("August 2006")
...
predictors_grid  <-  scale(predictors_grid)
...
gamma_mcmc_array <- array(data = unlistaux(gamma_mcmc, niter, ncol(covs)), dim = c(Ngroups, niter,  ncol(covs)))
...
# saveRDS(y_c_list, "data/processed/posteriors_maps/posterior_ethyl_map.rds")

y_c_list <- readRDS("data/processed/posteriors_maps/posterior_ethyl_map.rds")
...
grid_mean$Campaign <- factor(grid_mean$Campaign, 
                             levels = c("means_dec", "means_apr", 
                                        "means_aug", "means_campaign"), 
                             labels = c("December 2005", "April 2006",
                                        "August 2006", "Mean"))


grid_sd$Campaign <- factor(grid_sd$Campaign, 
                           levels = c("sd_dec", "sd_apr", "sd_aug", "sd_campaigns"), 
                           labels = c("December 2005", "April 2006", 
                                      "August 2006", "Mean"))

# st_write(grid_mean[grid_mean$Campaign == "April 2006", ], "output/ethyl_mean_april.shp")
# st_write(grid_mean[grid_mean$Campaign == "December 2005", ], "output/ethyl_mean_december.shp")
# st_write(grid_mean[grid_mean$Campaign == "August 2006", ], "output/ethyl_mean_august.shp")
# 

plot_mean <- ggplot(grid_mean) + 
  geom_sf(aes(fill = Mean_hexane), colour = NA) +  
  ditch_the_axes + 
  labs(fill = expression ("Log Ethylbenzene")) + 
  geom_point(data = ethyl_predictors, 
             aes(x = X, y = Y), 
